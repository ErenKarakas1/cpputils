...
TEST_CASE("Arg stores options") {
    ...
    CHECK(args.size() == 2);
    CHECK(args[0].short_alias() == 'a');
    CHECK(args[1].name() == "name");
}
...
TEST_CASE("Print an example program help string") {
    ...
    cmd.arg(Arg::flag("help").short_alias('h').long_alias("help").about("Print this help message"));
...
TEST_CASE("multiple value arguments") {
    ...
    const char* argv[] = {"compile", "-I", "/usr/include", "-I", "/opt/include", "-D", "DEBUG", "-D", "VERBOSE", "main.cpp", "utils.cpp"};
    int argc = 11;
